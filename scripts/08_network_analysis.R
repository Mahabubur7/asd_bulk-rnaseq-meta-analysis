# PPI network analysis of ASD iPSC meta-analysis DEGs (ASD vs Healthy Control) (STRING + igraph)
# Load packages
library(httr)
library(igraph)
library(ggraph)
...
PADJ_CUTOFF     <- 0.05
LFC_CUTOFF      <- 1
STRING_SCORE     <- 400    # combined score threshold (0-1000); 400 = medium confidence
...
# randomP already comes from per-gene random-effects model — no second BH needed
meta <- meta |> mutate(rem_padj = randomP)
