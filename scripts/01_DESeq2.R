...
# Pin dplyr verbs to avoid masking issues
...
# Define GEO dataset IDs
geo_ids <- c("GSE67528", "GSE124308")

# Define color palette for PCA plots
custom_colors <- c("ASD" = "#de2d26", "Control" = "#2171b5",
                   "asd" = "#de2d26", "control" = "#2171b5",
                   "Healthy Control" = "#2171b5", "healthy control" = "#2171b5")
...
for (geo_id in geo_ids) {
  ...
  colData <- data.frame(
    condition = relevel(factor(ifelse(grepl("control|healthy|normal",
                                            tolower(as.character(metadata$condition))),
                                      "Control", "ASD")), ref = "Control"),
  ...
  ggsave(filename = paste0(pca_dir, "/", geo_id, "_pre.png"),
         plot = pca_pre, width = 5, height = 5, units = "in", bg = "white", dpi = 600)

  ggsave(filename = paste0(pca_dir, "/", geo_id, "_post.png"),
         plot = pca_post, width = 5, height = 5, units = "in", bg = "white", dpi = 600)
}
